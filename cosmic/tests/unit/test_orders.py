...
@pytest.fixture()
def batch():
    batch_data = {"reference": "mybatchreference", "sku": "BLUE-VASE", "quantity": 20}
...
def test_get_from_stock_when_possible():
    ...
    batch_in_stock = Batch(
        reference="batch-in-stock-reference", sku="BLUE-VASE", quantity=20
    )
...
def test_get_from_stock_with_smallest_eta():
    tomorrow = dt.date.today() + dt.timedelta(days=1)
    batch1 = Batch(reference="batch1", sku="BLUE-VASE", quantity=10, eta=tomorrow)
    batch2 = Batch(
        reference="batch2",
        sku="BLUE-VASE",
        quantity=10,
